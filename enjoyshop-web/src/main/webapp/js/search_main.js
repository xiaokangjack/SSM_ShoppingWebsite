...
SEARCH.page = function(b, a) {
    ...
    window.location = "http://www.enjoyshop.com/search.html?q="+SEARCH.query+"&page="+b;
//    if (a) {
    ...
    $.login({modal: true,complete: function(c) {
            if (c != null && c.IsAuthenticated != null && c.IsAuthenticated) {
                ...
                $.getJSON(d, function(f) {
                    if (f.code == 1) {
                        var e = confirm("如您已安装乐购LeBook客户端，请点击“确定”自动启动客户端\n如您尚未安装乐购LeBook客户端，请点击“取消”将引导您免费安装客户端");
                        if (e == true) {
                            window.location = "LEBK:///Bought"
                        } else {
                            if (confirm('如果您没有安装LeBook客户端，请点击"确定"按钮到乐购网站下载\n如果您不需要安装，请点击"取消"按钮')) {
